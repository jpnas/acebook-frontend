  ...
  return (
    <div className="space-y-8">
      <div className="rounded-3xl border bg-white/80 p-8 shadow-sm">
        <p className="text-sm font-semibold uppercase tracking-[0.3em] text-primary">AceBook</p>
        <h1 className="mt-4 text-3xl font-semibold text-foreground">Bem-vindo de volta!</h1>
        <p className="mt-2 text-muted-foreground">
          Selecione uma das opções abaixo para gerenciar suas reservas ou descobrir novas quadras.
        </p>
      </div>
      ...
      <section className="grid gap-6 md:grid-cols-2">
        {shortcuts.map((item) => {
          ...
          return (
            <Card key={item.title} className="border-none bg-white/90 shadow-sm">
              <CardHeader>
  ...
  return (
    <div className="space-y-6">
      <Card className="border-none bg-white/80 shadow-sm">
        <CardHeader>
          <CardTitle className="text-2xl">Bem-vindo ao painel do clube</CardTitle>
          <CardDescription>Escolha uma área para administrar operações diárias.</CardDescription>
        </CardHeader>
